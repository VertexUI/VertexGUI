...
public class StatePropertyWrapper<S> {
  ...
  var propertyBindings: [AnyKeyPath: AnyErasedInternalReactiveProperty] = [:]
...
public class ImmutableStateBindingProxy<StateWrapper: StatePropertyWrapper<State>, State> {
  ...
  public subscript<T>(dynamicMember keyPath: KeyPath<State, T>) -> ReactivePropertyProjection<T> {
    if stateWrapper.propertyBindings[keyPath] == nil {
      ...
      stateWrapper.propertyBindings[keyPath] = AnyErasedInternalReactiveProperty(wrapping: newBinding)
    }
