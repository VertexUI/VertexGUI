...
public class Aligner: Widget {
    ...
    override public func performLayout() {
        ...
        bounds.size = constraints!.constrain(maxSize)
        ...
        for child in children {
            if let child = child as? Alignable {
                ...
                case .Center:
                    child.bounds.min.x = bounds.size.width / 2 - child.bounds.size.width / 2
                case .End:
                    child.bounds.min.x = bounds.size.width - child.bounds.size.width
                }
