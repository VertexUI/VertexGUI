...
public class Border: SingleChildWidget {
  ...
  public init(
    top: Double = 0, right: Double = 0, bottom: Double = 0, left: Double = 0, color: Color,
  ...
  public convenience init(
    all: Double, color: Color, @WidgetBuilder child childBuilder: @escaping () -> Widget
  ) {
    ...
    RenderObject.Container {
      ...
      if borders.top > 0 {
        RenderObject.RenderStyle(strokeWidth: borders.top, strokeColor: FixedRenderValue(color)) {
          RenderObject.LineSegment(
            from: globalBounds.min + DVec2(0, borders.top / 2),
            to: globalBounds.min + DVec2(bounds.size.width, borders.top / 2))
        }
      }
      ...
      if borders.right > 0 {
        RenderObject.RenderStyle(strokeWidth: borders.right, strokeColor: FixedRenderValue(color)) {
          RenderObject.LineSegment(
            from: globalBounds.min + DVec2(bounds.width - borders.right / 2, 0),
            to: globalBounds.min + DVec2(bounds.width - borders.right / 2, bounds.height))
        }
      }
      ...
      if borders.bottom > 0 {
        ...
        {
          RenderObject.LineSegment(
            from: globalBounds.min + DVec2(0, bounds.height - borders.bottom / 2),
            to: globalBounds.min + DVec2(bounds.width, bounds.height - borders.bottom / 2))
        }
      }
      ...
      if borders.left > 0 {
        RenderObject.RenderStyle(strokeWidth: borders.left, strokeColor: FixedRenderValue(color)) {
          RenderObject.LineSegment(
            from: globalBounds.min - DVec2(borders.left / 2, 0),
            to: globalBounds.min + DVec2(-borders.left / 2, bounds.height))
        }
      }
    }
  }
