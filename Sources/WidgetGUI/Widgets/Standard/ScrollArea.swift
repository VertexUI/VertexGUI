...
public class ScrollArea: SingleChildWidget, GUIMouseEventConsumer {
  ...
  private var maxOffsets: DVec2 {
    DVec2(child.bounds.size - bounds.size)
  ...
  private var xScrollBarBounds: DRect {
    DRect(
      min: DVec2(scrollBarTranslations.x, bounds.size.height - scrollBarWidths.x),
      size: DSize2(scrollBarLengths.x, scrollBarWidths.x))
  }

  private var yScrollBarBounds: DRect {
    DRect(
      min: DVec2(bounds.size.width - scrollBarWidths.y, scrollBarTranslations.y),
      size: DSize2(scrollBarWidths.y, scrollBarLengths.y))
  }
  ...
  override public final func performLayout(constraints: BoxConstraints) -> DSize2 {
    ...
    scrollBarLengths.y =
      constrainedSize.height / (child.bounds.size.height / constrainedSize.height)
